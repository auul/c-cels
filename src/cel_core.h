...
#define CEL_CAT3_0(a, b, c) a##b##c
#define CEL_CAT3(a, b, c) CEL_CAT3_0(a, b, c)
...
CEL_STATIC_ASSERT(
	sizeof(void *) <= sizeof(cel_uptr),
	"Pointer size could not be determined during preprocessing", bad_uptr_size);
...
#ifdef __SIZE_MAX__
#	define CEL_SIZE_MAX __SIZE_MAX__
#elif CEL_HAS_INT64
#	define CEL_SIZE_MAX 0xffffffffffffffffull
#else
#	define CEL_SIZE_MAX 0xfffffffful
#endif

#if CEL_SIZE_MAX == 0xffu
#	define CEL_SIZE_BYTES 1
#elif CEL_SIZE_MAX == 0xffffu
#	define CEL_SIZE_BYTES 2
#elif CEL_SIZE_MAX == 0xfffffffflu
#	define CEL_SIZE_BYTES 4
#elif CEL_SIZE_MAX == 0xffffffffffffffffllu
#	define CEL_SIZE_BYTES 8
#else
#	define CEL_SIZE_BYTES 0
#endif

CEL_STATIC_ASSERT(
	CEL_SIZE_BYTES, "Could not determine size of size_t", unknown_size_t_size);
...
#	if CEL_MSVC
#		include <immintrin.h>
#		define CEL_CPU_RELAX() _mm_pause()
#	else
#		define CEL_CPU_RELAX() __asm__ __volatile__("pause")
#	endif
#elif CEL_ARM32 || CEL_ARM64
#	if defined(__has_builtin)
#		if __has_builtin(__builtin_arm_yield)
#			define CEL_CPU_RELAX() __builtin_arm_yield()
#		else
#			define CEL_CPU_RELAX() __asm__ __volatile__("yield")
#		endif
#	else
#		define CEL_CPU_RELAX() __asm__ __volatile__("yield")
#	endif
#else
#	define CEL_CPU_RELAX() ((void)0)
#endif
...
#if defined(_WIN32)
#	include <windows.h>
#	define CEL_YIELD() Sleep(0)
#elif defined(_POSIX_VERSION) || defined(_POSIX_THREADS)
#	include <sched.h>
#	define CEL_YIELD() sched_yield()
#else
#	define CEL_YIELD() ((void)0) /* no-op if unknown */
#endif

/* Exponential Backoff Polling */

#define CEL_POLL(init, cap, test)                                              \
	do {                                                                       \
		unsigned cel_pvt_spin_init = init;                                     \
		unsigned cel_pvt_spin_cap = cap;                                       \
		unsigned cel_pvt_spins = cel_pvt_spin_init;                            \
		for (;;) {                                                             \
			int cel_pvt_retval = (test);                                       \
			if (cel_pvt_retval < 0) {                                          \
				return cel_pvt_retval;                                         \
			} else if (!cel_pvt_retval) {                                      \
				break;                                                         \
			}                                                                  \
                                                                               \
			unsigned cel_pvt_k = cel_pvt_spins;                                \
			while (cel_pvt_k--) {                                              \
				CEL_CPU_RELAX();                                               \
			}                                                                  \
                                                                               \
			if (cel_pvt_spins < cel_pvt_spin_cap) {                            \
				unsigned cel_pvt_spin_next = cel_pvt_spins << 1;               \
				cel_pvt_spins = (cel_pvt_spin_next > cel_pvt_spin_cap ||       \
								 cel_pvt_spin_next < cel_pvt_spins)            \
									? cel_pvt_spin_cap                         \
									: cel_pvt_spin_next;                       \
			} else {                                                           \
				cel_pvt_retval = (test);                                       \
